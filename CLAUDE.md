...
2. **`Database`** — thin wrapper around a single sqlite3 connection. Schema is created idempotently in `_init_schema()` (`CREATE TABLE IF NOT EXISTS`), and default preference rows are seeded once in `_seed_prefs()`. Tables: `sessions` (the actual logged practice sessions), `templates` (reusable session presets for quick logging / CLI use), `pref_languages`, `pref_activity_types`, `pref_specific_activities` (all user-configurable enable/disable lists driven from the Settings tab), `pref_activity_colors` (per-activity chart colors), and `pref_current_level` (per language+system assumed-current-level, e.g. `("ja", "CEFR") -> "B1"`, edited on Projection's "Current Level" sub-tab via `get_current_levels`/`save_current_levels`). `language_time_stats(lang_code)` aggregates total minutes / first / last session date for one language, feeding the Projection tab; `earliest_session_date(lang_code=None)` backs the Stats/Cumulative "All time" quick-range preset. All queries are parameterized — follow that pattern for any new query.

3. **`FilterBar`** — shared widget (language/date-range/grouping filter + quick-range presets) reused by the Stats and Cumulative tabs; picking a quick-range preset, changing the "By:" grouping combobox, or picking a Language calls `on_refresh` immediately, while the From/To date fields wait for the "Refresh" button. Most presets are static date math, but "All time" queries `Database.earliest_session_date` for the currently-selected language (or across all languages if none is picked). `_load_chart_context(db, filter_bar)` is the shared refresh step both chart tabs call: it parses the filter bar's date range (showing the error dialog and returning `None` if invalid), fetches rows via `Database.get_chart_rows`, and runs them through `_prepare_chart_data`. `_summary_line(rows, p_keys, grouping)` builds the "Sessions: N · Total: ... · Avg session: ... · Avg per {grouping}: ... · Top activity: ..." strip both tabs display above their chart.
...
5. **Tabs**, each a `ttk.Frame` subclass, added to a single `ttk.Notebook` in `LanguageLoggerApp`:
   ...
   - `LearningTimeTab` — has two inner sub-tabs sharing its own `ttk.Notebook`: "Learning Time" is the static reference table (FSI-based language difficulty/time estimates), sortable via `_sort_treeview`, highlighting rows for languages the user has actually logged (`Database.distinct_languages()`); "Vocabulary" is `VocabTab`, built directly against this tab's inner `Notebook` rather than the app's top-level one — a static reference table of approximate vocabulary size per proficiency level (CEFR for every language, plus HSK/JLPT/TOPIK where applicable), filterable to the user's enabled languages (`Database.pref_languages()`). `LearningTimeTab.refresh()` also calls `self.tab_vocab.refresh()`, and `LanguageLoggerApp._on_prefs_changed` reaches it via `self.tab_learntime.tab_vocab.refresh()`.
   - `ProjectionTab` — for languages the user has actually logged sessions for, has three inner sub-tabs sharing one language filter and a Day/Week/Month/Year pace slider: "To Reach Level" projects estimated remaining duration and a target date to reach each level via `_project_levels` (scales hours-needed off the language's FSI `hours_est`, anchored so CEFR C1 == that FSI hour estimate); "Per-Level Effort" shows the incremental words/hours/duration to go from each level to the next via `_incremental_levels`; "Current Level" is `CurrentLevelTab`, built directly against Projection's inner `Notebook` rather than the app's top-level one — a scrollable list of per-language/system dropdowns (CEFR plus native scale where one exists) that saves to `pref_current_level`, calling back into `ProjectionTab._populate` (via its `on_saved` constructor arg) so the other two sub-tabs pick up the change immediately. A "Pace used for projections" radio choice feeds either the historical average (`total_minutes` from `Database.language_time_stats` spread over the days since the first session) or a custom assumed minutes/day into `_project_levels`' `avg_daily_minutes` param, and also into the "Per-Level Effort" days-needed math. Whichever assumed current level is set for a (language, system) pair is passed as `_project_levels`' `assumed_level`, representing where the user already was *before* they started logging sessions — its hours-needed is *added* to actually-logged hours (not just used as a floor), so levels at or below it always read "reached" in both sub-tabs (tagged `"reached"`/shown as "Done") regardless of actual logged hours, and logged hours accumulate on top of it for levels above it. Both sub-tabs run once for CEFR and again for the language's native scale (HSK/JLPT/TOPIK) via `NATIVE_LEVEL_SYSTEMS`, shown in a "System" column — `_project_levels`/`_incremental_levels` take a `word_counts` table so either can be scaled the same way. The slider's chosen unit rescales the pace figure and the remaining/needed duration (via `_format_days_in_unit`) but not the target date.
