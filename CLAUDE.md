...
`test_main.py` covers the date/formatting helpers, the `Database` layer (temp sqlite files, never the real `sessions.db`), the reference data tables (FSI/CEFR/HSK/JLPT/TOPIK — guards against typos in those hand-maintained dicts), the `_sort_treeview` Treeview-sort helper, and an end-to-end CLI smoke test that shells out to `python main.py --template ...`. The Treeview-sort tests are skipped automatically when no `$DISPLAY` is available. It intentionally does not drive the Tkinter tab classes themselves (`LogTab`, `HistoryTab`, etc.) — those are exercised manually.
...
1. **Module-level constants**: `LANGUAGES` (code→name map), `ACTIVITIES` (activity type → list of specific activities), `_CHART_COLORS`, `FSI_CATEGORIES`/`LANGUAGE_DIFFICULTY` (FSI language-difficulty reference data used by the Learning Time tab), and `CEFR_WORD_COUNTS`/`NATIVE_LEVEL_SYSTEMS` (CEFR/HSK/JLPT/TOPIK vocabulary-size reference data used by the Vocabulary tab). Free functions above the classes (`_fmt_time`, `_period_key`, `_all_periods`, `_extract_lang_code`, `_lang_display`, `_prepare_chart_data`, `_load_chart_context`, `_validate_session_fields`, `_build_mpl_frame`, `_sort_treeview`) are shared helpers used across multiple tabs — check here before adding a new helper, since date/period/formatting/sorting logic is centralized rather than duplicated per-tab.
...
5. **Tabs**, each a `ttk.Frame` subclass, added to a single `ttk.Notebook` in `LanguageLoggerApp`:
   ...
   - `StatsTab` — stacked bar + pie chart over a date range (via `FilterBar` + `_load_chart_context`).
...
6. **`LanguageLoggerApp`** (the `tk.Tk` root) — wires all tabs into one `ttk.Notebook`, preloads matplotlib on a background thread (`_preload_matplotlib`) so the first chart render isn't slow, binds Ctrl+S to save-in-current-tab, and refreshes tab-specific data on `<<NotebookTabChanged>>` (History/Stats/Cumulative/Learning Time all lazily refresh only when selected rather than eagerly; Vocabulary instead refreshes from `_on_prefs_changed` since its data only depends on language prefs, not on sessions).
